...
if ($data->num_rows > 0) {
    while ($row = $data->fetch_assoc()) {
        ...
        $commentQuery = "SELECT id, comment, created_at, user_id FROM comments WHERE events_id = $events_id";
       ...
       <div class="flex flex-col" style="padding-left: 0; background-image: none; -webkit-appearance: none;">
        ...
        while ($commentRow = mysqli_fetch_assoc($commentResult)) {
            $commentUserQuery = "SELECT name FROM users WHERE id = " . $commentRow["user_id"];
            $commentUserResult = mysqli_query($deneme, $commentUserQuery);
            $commentUser = mysqli_fetch_assoc($commentUserResult);

            echo '
                <div class="my-4">
                    <div class="text-gray-400 text-md">Yorum Yapan: ' . htmlspecialchars($commentUser["name"]) . '</div>
            ...
            $replyQuery = "SELECT id, reply_comment, created_at, user_id FROM replies WHERE comment_id = " . $commentRow["id"];
            $replyResult = mysqli_query($deneme, $replyQuery);

            while ($replyRow = mysqli_fetch_assoc($replyResult)) {
                $replyUserQuery = "SELECT name FROM users WHERE id = " . $replyRow["user_id"];
                $replyUserResult = mysqli_query($deneme, $replyUserQuery);
                $replyUser = mysqli_fetch_assoc($replyUserResult);

                echo '
                    <div class="my-1 ml-4">
                        <div class="text-gray-400 text-md">Cevaplayan: ' . htmlspecialchars($replyUser["name"]) . '</div>
